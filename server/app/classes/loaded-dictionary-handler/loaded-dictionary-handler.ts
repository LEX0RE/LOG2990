...
export class LoadedDictionaryHandler {
    ...
    modifyDictionaryServer(newDictionary: ClientDictionary) {
        const dictionaryServer = this.dictionaries.find((dictionary: Dictionary) => dictionary.dictionaryId === newDictionary.dictionaryId);

        if (dictionaryServer) {
            const index = this.dictionaries.indexOf(dictionaryServer);

            this.dictionaries[index].title = newDictionary.title;
            this.dictionaries[index].description = newDictionary.description;
        }
    }
