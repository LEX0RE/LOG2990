...
export class WordsFind {
    ...
    async fastActions(easel: Easel, game: Game, end: EndSearching = { found: INDEX_NOT_FOUND }): Promise<Hint[]> {
        ...
        // eslint-disable-next-line no-console -- Permet de voir la performance de l'algorithme
        console.log(`${this.result.length} words found in ${Date.now() - this.startTime} ms with !indice`);
