...
export class DictionaryService {
    ...
    async modifyDictionary(newDictionary: ClientDictionary, oldTitle: string): Promise<void> {
        this.loadedDictionariesHandler.modifyDictionaryServer(newDictionary);
        this.modifyAvailableDictionary(newDictionary);

        const dictionary = await this.fileHandler.getDictionaryWithAll(oldTitle);

        dictionary.title = newDictionary.title;
        dictionary.description = newDictionary.description;
        await this.fileHandler.deleteDictionary(oldTitle);
        await this.fileHandler.setDictionary(dictionary);
    }

    async deleteDictionary(dictionaryId: number): Promise<void> {
        const dictionary = this.findAvailableDictionaryId(dictionaryId);

        if (dictionary) {
            const index = this.availableDictionaries.indexOf(dictionary);

            this.availableDictionaries[index].available = false;
            await this.fileHandler.deleteDictionary(dictionary.title);
        }
    }

    async deleteAll(): Promise<void> {
        const dictionary = this.findAvailableDictionaryTitle(DEFAULT_DICTIONARY_TITLE);

        this.availableDictionaries.forEach((dictionaryAvailable: AvailableDictionary) => (dictionaryAvailable.available = false));
        if (dictionary) {
            const index = this.availableDictionaries.indexOf(dictionary);

            this.availableDictionaries[index].available = true;
        }
        this.fileHandler.deleteAllFiles();
    }
    ...
    private modifyAvailableDictionary(newDictionary: ClientDictionary) {
        const dictionaryAvailable = this.findAvailableDictionaryId(newDictionary.dictionaryId);

        if (dictionaryAvailable) {
            const index = this.availableDictionaries.indexOf(dictionaryAvailable);

            this.availableDictionaries[index].title = newDictionary.title;
            this.availableDictionaries[index].description = newDictionary.description;
        }
    }
