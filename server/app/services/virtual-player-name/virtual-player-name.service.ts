...
export class VirtualPlayerNameService {
    ...
    async addName(difficulty: string, name: CommonVirtualPlayerName): Promise<void> {
        this.difficultyToCollection(difficulty);
        await this.collection.insertOne(name);
    }

    async modifyName(difficulty: string, oldName: string, newName: string): Promise<void> {
        this.difficultyToCollection(difficulty);
        return this.collection.updateOne({ playerName: oldName }, { $set: { playerName: newName } }).then(async () => Promise.resolve());
    }

    async deleteAll(): Promise<void> {
        await this.deleteCollection(Difficulty.Easy);
        await this.deleteCollection(Difficulty.Hard);
    }

    async deleteName(difficulty: string, name: string): Promise<void> {
        this.difficultyToCollection(difficulty);
        await this.collection.deleteOne({ playerName: name });
    }

    private async deleteCollection(difficulty: string): Promise<void> {
        this.difficultyToCollection(difficulty);
