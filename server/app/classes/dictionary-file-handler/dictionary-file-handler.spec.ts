...
describe('DictionaryFileHandler', () => {
    ...
    beforeEach(() => {
        ...
        unlinkStub = stub(fsAsync, 'unlink').resolves();
    });
    ...
    it('deleteDictionary should throw if unlink fails', async () => {
        ...
        unlinkStub.rejects(new Error(errorMessage));

        await fileHandler
            .deleteDictionary(FAKE_DICTIONARY_TITLE)
            .then(() => expect(false).to.be.eql(true))
            .catch((error: Error) => expect(error.message).to.be.eql(errorMessage));
    });

    it('deleteAll should unlink files', async () => {
        unlinkStub.resolves(undefined);
        readDirStub.resolves(FAKE_FILES_NAMES());

        await fileHandler.deleteAllFiles();
        assert.calledWith(unlinkStub, DICTIONARY_PATH + FAKE_FILES_NAMES()[0]);
    });

    it('deleteAll should throw an error when unlink throws', async () => {
        const errorMessage = 'unlink failed';

        unlinkStub.rejects(new Error(errorMessage));
        readDirStub.resolves(FAKE_FILES_NAMES());

        expect(fileHandler.deleteAllFiles()).to.eventually.be.rejectedWith(errorMessage);
    });
