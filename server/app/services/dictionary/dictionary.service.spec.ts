...
import { DEFAULT_DICTIONARY_TITLE } from '@app/constants/dictionary';
...
describe('DictionaryService', () => {
    ...
    it('modifyDictionary should call modifyDictionaryServerSpy and modifyAvailableDictionarySpy', async () => {
        fileHandler.getDictionaryWithAll.resolves(FAKE_DICTIONARY_WITH_ALL());
        // eslint-disable-next-line @typescript-eslint/no-explicit-any -- stub membre prive
        const modifyAvailableDictionarySpy = stub(dictionaryService, 'modifyAvailableDictionary' as any);

        const newDictionary = FAKE_DICTIONARY_WITH_ALL();

        newDictionary.title = 'newTitle';
        newDictionary.description = 'newDescription';

        await dictionaryService.modifyDictionary(newDictionary, FAKE_DICTIONARY_TITLE);
        assert.calledOnce(modifyAvailableDictionarySpy);
    });

    it('modifyDictionary throw if delete fails', async () => {
        const errorMessage = 'unlink failed';

        fileHandler.getDictionaryWithAll.resolves(FAKE_DICTIONARY_WITH_ALL());
        fileHandler.deleteDictionary.rejects(new Error(errorMessage));

        expect(dictionaryService.modifyDictionary(FAKE_DICTIONARY_WITH_ALL(), FAKE_DICTIONARY_TITLE)).to.eventually.rejectedWith(errorMessage);
    });

    it('modifyDictionary throw if set fails', async () => {
        const errorMessage = 'unlink failed';

        fileHandler.getDictionaryWithAll.resolves(FAKE_DICTIONARY_WITH_ALL());
        fileHandler.deleteDictionary.resolves();
        fileHandler.setDictionary.rejects(new Error(errorMessage));

        expect(dictionaryService.modifyDictionary(FAKE_DICTIONARY_WITH_ALL(), FAKE_DICTIONARY_TITLE)).to.eventually.rejectedWith(errorMessage);
    });

    it('deleteDictionary should set available to false', async () => {
        const dictionaryId = 3;

        dictionaryService['availableDictionaries'][0].dictionaryId = dictionaryId;

        dictionaryService.deleteDictionary(dictionaryId);

        expect(dictionaryService['availableDictionaries'][0].available).to.be.eql(false);
    });

    it('deleteDictionary should not set available if find is undefined', async () => {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any -- méthode privée
        stub(dictionaryService, 'findAvailableDictionaryId' as any).returns(undefined);
        dictionaryService.deleteDictionary(FAKE_DICTIONARY_ID);
        expect(dictionaryService['availableDictionaries'][0].available).to.be.eql(false);
    });

    it('deleteDictionary should throw if unlink fails', async () => {
        const errorMessage = 'unlink failed';

        fileHandler.deleteDictionary.rejects(new Error(errorMessage));

        expect(dictionaryService.deleteDictionary(FAKE_DICTIONARY_ID)).to.eventually.rejectedWith(errorMessage);
    });

    it('deleteAll should set available to false for all dictionaries', async () => {
        dictionaryService.deleteAll();

        dictionaryService['availableDictionaries'].forEach((dictionary: AvailableDictionary) => {
            expect(dictionary.available).to.be.eql(false);
        });
    });

    it('deleteAll should set available to false for all dictionaries except default one', async () => {
        dictionaryService['availableDictionaries'][0].title = DEFAULT_DICTIONARY_TITLE;
        dictionaryService.deleteAll();

        dictionaryService['availableDictionaries'].forEach((dictionary: AvailableDictionary) => {
            if (dictionary.title === DEFAULT_DICTIONARY_TITLE) expect(dictionary.available).to.be.eql(true);
            else expect(dictionary.available).to.be.eql(false);
        });
    });
    ...
    it('modifyAvailableDictionary should not  if invalid dictionary id', () => {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any -- méthode privée
        stub(dictionaryService, 'findAvailableDictionaryId' as any).returns(undefined);
        dictionaryService['modifyAvailableDictionary'](FAKE_CLIENT_DICTIONARY());
        expect(dictionaryService['availableDictionaries'][0].title).to.be.eql(FAKE_DICTIONARY_TITLE);
    });
