...
import { DEFAULT_DICTIONARY_PATH, DEFAULT_DICTIONARY_TITLE, DICTIONARY_FORMAT, DICTIONARY_PATH, FILE_ENCODING } from '@app/constants/dictionary';
...
export class DictionaryFileHandler {
    ...
    async deleteDictionary(title: string): Promise<void> {
        return fsAsync.unlink(DICTIONARY_PATH + title + DICTIONARY_FORMAT);
    }
    ...
    async deleteAllFiles(): Promise<void> {
        fsAsync.readdir(DICTIONARY_PATH).then((files) => {
            files.splice(
                files.findIndex((file) => file === DEFAULT_DICTIONARY_TITLE + DICTIONARY_FORMAT),
                1,
            );
            files.forEach(async (file) => fsAsync.unlink(DICTIONARY_PATH + file));
        });
    }
