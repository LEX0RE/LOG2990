...
import { COLOR_BACKGROUND, COLOR_LETTER, DEFAULT_FONT_SIZE } from '@app/constants/font-letter';
import {
    DEFAULT_RADIUS,
    FONT_LETTER_SQUARE,
    MINUS_POINT_SQUARE,
...
export class GridPlaceLetterService {
    ...
    createLetterSquare(position: Vector2D, letter: CommonLetter): void {
        ...
        this.gridContext.gridContext.fillStyle = COLOR_BACKGROUND;
    ...
    private isNewPosition(position: Vector2D, letter: CommonLetter): void {
        if (
            !this.gridContext.saveLetterSquare.find((letterSquare: PositionLetter) =>
                this.gridContext.comparePosition(position, letterSquare.position),
            )
        )
