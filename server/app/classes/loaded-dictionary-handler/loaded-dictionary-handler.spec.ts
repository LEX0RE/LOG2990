...
describe('LoadedDictionaryHandler', () => {
    ...
    it('modifyDictionaryServer should modify dictionary in dictionaries', async () => {
        const newTitle = 'newTitle';
        const newDescription = 'New description';
        const fakeDictionaryClient = { title: newTitle, description: newDescription, dictionaryId: FAKE_DICTIONARY_ID };

        loadedDictionaryHandler.modifyDictionaryServer(fakeDictionaryClient);
        // eslint-disable-next-line dot-notation -- méthode privée
        expect(loadedDictionaryHandler['dictionaries'][0].title).to.be.eql(newTitle);
        // eslint-disable-next-line dot-notation -- méthode privée
        expect(loadedDictionaryHandler['dictionaries'][0].description).to.be.eql(newDescription);
        // eslint-disable-next-line dot-notation -- méthode privée
        expect(loadedDictionaryHandler['dictionaries'][0].dictionaryId).to.be.eql(FAKE_DICTIONARY_ID);
    });

    it('modifyDictionaryServer should not modify dictionary in dictionaries if find is undefined', async () => {
        // eslint-disable-next-line dot-notation -- méthode privée
        ...
        loadedDictionaryHandler.modifyDictionaryServer(fakeDictionaryClient);
        // eslint-disable-next-line dot-notation -- méthode privée
        expect(loadedDictionaryHandler['dictionaries'][0].title).to.be.eql(FAKE_DICTIONARY_TITLE);
        // eslint-disable-next-line dot-notation -- méthode privée
        expect(loadedDictionaryHandler['dictionaries'][0].description).to.be.eql(FAKE_DICTIONARY_DESCRIPTION);
        // eslint-disable-next-line dot-notation -- méthode privée
        expect(loadedDictionaryHandler['dictionaries'][0].dictionaryId).to.be.eql(FAKE_DICTIONARY_ID);
    });
