...
export class GridMouseEventView {
    ...
    updateView(): void {
        ...
        this.grid.placement.letters.forEach((letter) => {
        ...
        this.updatePositionPlacement(positionPlacement, letterInGrid);
    }
    ...
    private updatePositionPlacement(positionPlacement: Vector2D, letterInGrid: PositionLetter[]): void {
    ...
    private nextPosition(positionPlacement: Vector2D): Vector2D {
    ...
    private isGoodPosition(position: Vector2D): boolean {
        ...
        this.grid.saveLetterSquare.forEach((letter) => {
            if (this.grid.comparePosition(position, letter.position)) {
                const letterFromUs = this.letterInGrid.find((letterPlace) => this.grid.comparePosition(position, letterPlace.position));
