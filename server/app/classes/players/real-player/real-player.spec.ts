...
describe('RealPlayer', () => {
    ...
    beforeEach(() => {
        ...
        player1 = new RealPlayer(FAKE_PLAYER_1_NAME, FAKE_SOCKET_ID_PLAYER_1);
        player2 = new RealPlayer(FAKE_PLAYER_2_NAME, FAKE_SOCKET_ID_PLAYER_2);
    });

    afterEach(() => restore());

    it('should create a simple player', () => {
        expect(player1).to.ownProperty('name', FAKE_PLAYER_1_NAME);
    });
    ...
    it('outsideResolve should resolve with temp ecoule if activeGame', async () => {
    ...
    it('outsideResolve should resolve with temp ecoule if activeGame - handle outsideResolve undefined', async () => {
        stubs.gameManager.getGameByPlayerId.returns(undefined);
        stubs.gameplay.addToPlayerTurnQueue.callsFake(async () => {
            player1.outsideEndTime();
            return Promise.resolve();
